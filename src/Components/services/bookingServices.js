...
// tickets than are available. See event-data-schema.md section 4.
...
export const bookingService = {
  ...
  // Returns { success, booking?, error? } — components should check
  // `success` rather than assuming this always works.
  // For paid events, call this only AFTER paymentService confirms success —
  // this function itself does no payment processing, just persistence.
  create({ eventId, quantity, attendeeName, attendeeEmail, paymentMethod, paymentReference }) {
    ...
    // Two separate writes (booking, then event.quantityBooked) means a
    // partial-failure state is possible: booking saved, availability not
    // decremented. If the second write fails, roll the first one back
    // rather than leaving a "confirmed" booking that never reserved a seat.
  ...
  cancel(bookingId) {
    ...
    if (event) {
      ...
      if (!updatedEvent) {
        // Roll back the cancellation too — better to leave the booking
        // confirmed than to free up a seat that was never actually released.
